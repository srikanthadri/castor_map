...
if show_suggested and not suggested_gdf.empty:
    folium.GeoJson(
        suggested_gdf,
        style_function=lambda x: style_location(x, "pink"),
        tooltip=GeoJsonTooltip(fields=["id", "acreage"], aliases=["Location ID:", "Acreage (ha):"]),
...
for _, row in filtered_polygons.iterrows():
    ...
    color = "pink" if row["id"] <= 10 else "blue"

    if (color == "pink" and show_suggested) or (color == "blue" and show_existing):
        # Small circle marker
        folium.CircleMarker(
...
# st.sidebar.subheader("Polygon Layer Controls")
# show_existing = st.sidebar.checkbox("Show Existing Locations (Red)", value=True)
# show_suggested = st.sidebar.checkbox("Show Suggested Locations (pink)", value=True)
